# Draft README rewrite for v0.8

Per Obj 6 in the v0.8 plan. Replaces the current `README.md` (294 lines).
Targets: ~140 lines, opener leads with cost-of-status-quo, single
comparison table to abtop, "verified in CI" appears once as proof.

**Surfaced here for review before replacing README.md** because:
- Public competitive comparison with abtop (their stars, their feature
  shape) is a claim that should land only with your approval.
- The line "Proposed as a shared standard with abtop" references the
  interop issue that hasn't been posted yet — best to gate the README
  on the issue posting status.

**Two ways to resolve:**

1. **You approve as-is** → I overwrite `README.md` with this exact text
   (or you do it yourself with `mv docs/readme-draft.md README.md`).
2. **You want edits** → tell me what to change; I revise this draft.
